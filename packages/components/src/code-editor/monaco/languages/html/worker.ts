...
import type { Options } from './contribution'
...
export class HTMLWorker {
  ...
  doValidation(): Promise<htmlService.Diagnostic[]> {
    // not yet suported
  ...
  doComplete(
    ...
    const items = htmlCompletionPlugins
      .map(plugin => plugin.completions({ document, html: htmlDocument, position }))
      // .flat()
  ...
  doHover(uri: string, position: htmlService.Position): Promise<htmlService.Hover> {
    const document = this._getTextDocument(uri)
    const htmlDocument = this._languageService.parseHTMLDocument(document)
